...
```{r}
Pictures <- read.csv("ForagingOrder.csv") 
Samples <- 
Fixations <- merge(Pictures, Fixations)
Saccades <- merge(Pictures, Saccades)
```

## Flip the data on the y-axis since it's mirrored

```{r}
Samples <- Samples %>% mutate(PositionY = 1050-PositionY)
Fixations <- Fixations %>% mutate(PositionY = 1050-PositionY)
Saccades <- Saccades %>% mutate(StartY = 1050-StartY)
```
...
```{r}
make_scanpath("Emilie1", 2)
```

And here is one from the Count condition:

```{r}
make_scanpath("Emilie1", 7)
```
...
make_heatmap <- function(participant, trial){
  ...
  img <- jpeg::readJPEG(paste0('stimuli_Foraging/', x$Stimulus[1])) 
...
```{r}
make_heatmap("Emilie1", 8)
```

And one more:

```{r}
make_heatmap("Emilie1", 4)
```
...
ggplot(Fixations[Fixations$Trial == 8,], aes(PositionX, PositionY, color = FixationNo)) + 
    annotation_custom(img, xmin = 0, xmax = 1680, ymin = 0, ymax = 1050) +
    annotate(geom = "rect", xmin = 0, xmax = 1680, ymin = 0, ymax = 1050, fill = "white", alpha = .3) +
    # instead of plotting circles, we plot a density function, so that areas with different data density levels
    # have different colors. The sqrt(sqrt()) thing is just to get the ratio of warm/cold colors right
