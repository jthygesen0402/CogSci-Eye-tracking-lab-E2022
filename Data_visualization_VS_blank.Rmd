...
```{r}
Pictures <- read_csv("ForagingOrder.csv")
Fixations <- merge(Fixations, Pictures)
Saccades <- merge(Saccades, Pictures)
```
...
Saccades <- Saccades %>% mutate(StartY = 1050-StartY, EndY = 1050-EndY)
```
...
```{r}
make_scanpath("Jakob2", 7)
```

And here is one from the Count condition:

```{r}
make_scanpath("Jakob2", 1)
...
x1 <- Fixations[Fixations$Trial == 8]
