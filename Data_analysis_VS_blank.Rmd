...
```{r load data}
Fixations <- read.csv("./Data/Fixed/Fixations.csv")
Saccades <- read.csv("./Data/Fixed/Saccades.csv")

Fixations <- Fixations %>% mutate(ParticipantID=as.factor(ParticipantID), Task=as.factor(Task))
Saccades <- Saccades %>% mutate(ParticipantID=as.factor(ParticipantID), Task=as.factor(Task))
...
```{r}
nozero <- Saccades %>% filter(Amplitude != 0)

mGaus <- lmer(Amplitude ~ Task + (1+Trial:Task|ParticipantID),
              data = nozero)

mLog <-glmer(Amplitude ~ Task + (1+Trial:Task|ParticipantID),
              data = nozero,
...
SacSum <- Saccades %>% group_by(ParticipantID, Task, Trial) %>% summarise(AmpSum=sum(Amplitude)) 



m1 <- lmer(AmpSum ~ Task + (1|ParticipantID),
     data = SacSum)
m2 <- lmer(AmpSum ~ Task + (1+Trial|ParticipantID),
     data = SacSum)
m3 <- lmer(AmpSum ~ Task + (1+Trial*Task|ParticipantID),
     data = SacSum)
anova(m1,m2,m3)
