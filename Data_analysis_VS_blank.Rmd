...
```{r load data}
Fixations <- read_csv("./Data/Fixed/Fixations.csv")
Saccades <- read_csv("./Data/Fixed/Saccades.csv")
...
```{r}
#Saccade amplitudes for the 2 tasks
Saccades %>% ggplot(
  aes(x = Amplitude, color = Task))+
  geom_density()
...
#visually
Saccades %>% ggplot(
  aes(x = Task, y = Amplitude))+
  geom_boxplot()+
  stat_summary(fun.y = mean, geom = "point", shape = 4, size =1)
...
```{r}
exc_zero_sacc <- Saccades %>% filter(Amplitude != 0)

mGaus <- glmer(Amplitude ~ Task + (1 + Task|ParticipantID), data = Saccades, family = gaussian)


mLog <-  glmer(Amplitude ~ Task + (1 + Task|ParticipantID), data = exc_zero_sacc, family = gaussian(link = log))
...
```{r}
#Total amplitude
SacSearch <- Saccades %>% filter(Task == "Searching")
SacCount <- Saccades %>% filter(Task == "Counting")

as.numeric(SacSearch$Amplitude)

oga <- SacSearch %>% group_by(ParticipantID) %>% summarise(sum = sum(SacSearch$Amplitude))

oga

#Trying again
Saccades2 <- Saccades %>% 
  group_by(ParticipantID, Task, Trial) %>% 
  summarise(SumAmp = sum(Amplitude))

m1 <- lmer(SumAmp~Task+(1|ParticipantID), Saccades2)
m2 <- lmer(SumAmp~Task+(1+Trial|ParticipantID), Saccades2)
m3 <- lmer(SumAmp~Task+(1+Trial*Task|ParticipantID), Saccades2)
...
```{r}
par(mfrow=c(1,2))
plot(density(Fixations[Fixations$Task == "Searching", ]$Duration), main = "Search", xlim = c(0, 1000))
plot(density(Fixations[Fixations$Task == "Counting", ]$Duration), main = "Count", xlim = c(0, 1000))
```
